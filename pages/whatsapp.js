...
export default function WhatsApp({ theme, toggleTheme }) {
  ...
  const run = () => {
    alert(
      "Start WhatsApp verification. single:" +
        single +
        " bulkLen:" +
        bulk.split("\n").length +
        " file:" +
    ...
    <Layout theme={theme} toggleTheme={toggleTheme}>
      ...
      >
        ...
        <div className="glass p-6 rounded-2xl border border-white/10 shadow-xl space-y-4">
          <div>
            <label className="block text-sm mb-1">Single WhatsApp number</label>
            <input
              value={single}
